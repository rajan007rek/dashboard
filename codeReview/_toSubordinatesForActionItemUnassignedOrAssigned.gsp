   ...
   <table style="width:70%;border:1px solid #ddd">
        <tbody>
            <tr>
			 ...
			 <tr>
                <td style="color: #333; font-size:13px; padding: 21px 10px 12px 10px;">  Hi ${user.firstName},</td>
            </tr>
            <tr>
				  <td style="padding: 25px 9px 11px 30px;line-height: 18px;"> 
				    ${currentUser.firstName} ${currentUser.lastName} has <g:if test="${unassigned}">Unassigned</g:if><g:else>Assigned</g:else> you <g:if test="${unassigned}">from </g:if>the action item ${actionItem.actionItemId} in ${meeting.schedule.project?.name} for meeting commenced on
